...
class APIHandler:
    ...
    def __init__(self, logger: Logger, config: Config):
        ...
        self.categories_cache = None  # 缓存分类数据
    ...
    def fetch_categories(self) -> Optional[Dict[str, Any]]:
        """获取分类列表"""
        self.logger.info("开始获取分类列表")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('code') in [0, 200]:
                        self.categories_cache = data.get('data', {})
                        self.logger.info(f"成功获取分类列表，共 {len(self.categories_cache.get('data', []))} 个分类")
                        return self.categories_cache
                    self.logger.warning(f"获取分类失败 (第 {attempt + 1} 次): {data}")
                elif response.status_code in [401, 403]:
                    if self._handle_auth_error(response.status_code):
                        continue
                    else:
                        self.logger.error("重新登录失败，获取分类失败")
                        return None
                else:
                    self.logger.warning(f"获取分类失败 (第 {attempt + 1} 次): HTTP状态码 {response.status_code}")

            except Exception as e:
                self.logger.warning(f"获取分类请求失败 (第 {attempt + 1} 次): {e}")

            if attempt < self.config.max_retries - 1:
                time.sleep((attempt + 1) * 2)

        self.logger.error("获取分类失败，已达到最大重试次数")
        return None

    def find_category_by_name(self, title: str) -> Optional[int]:
        """根据标题查找匹配的主分类ID（只匹配父分类）"""
        if not self.categories_cache:
            self.fetch_categories()

        if not self.categories_cache:
            self.logger.warning("无法获取分类数据，使用默认分类ID 2")
            return 2

        categories = self.categories_cache.get('data', [])
        ...
        for parent_category in categories:
            ...
            # 检查主分类名称是否在标题中
            if parent_name and parent_name in title:
                parent_id = parent_category.get('id')
                self.logger.info(f"匹配到主分类: {parent_name} (ID: {parent_id})")
                return parent_id
